...
fn main() -> Result<(), Error> {
    let model = Model;
    let config = Config::new(500, 500);
    let mut app = App::new(model, config, update, view);
    app.run()
}
...
fn view(_model: &Model, time: f32) -> Vec<u8> {
    ...
    pixmap.fill_path(
        ...
        &paint1,
        FillRule::Winding,
        Transform::from_rotate_at(time * 15.0, 250.0, 250.0),
        None,
    );
    ...
    pixmap.stroke_path(
        ...
        &paint4,
        &stroke,
        Transform::from_rotate_at(-time * 25.0, 250.0, 250.0).post_scale(0.75, 0.75),
        None,
    );
